...
def fetch_emails(email_account, password, last_checked=None):
    """Fetch emails containing bills/invoices from Gmail with proper error handling"""
    ...
    try:
        ...
        search_criteria = []
        if last_checked:
            search_criteria.append(f'SINCE "{last_checked.strftime("%d-%b-%Y")}"')
        ...
        final_criteria = f'(OR {" ".join(search_criteria)})' if last_checked else f'(OR {" ".join(search_criteria)})'
        
        status, data = mail.search(None, final_criteria)
        if status != 'OK':
            st.error(f"Email search failed: {final_criteria}")
            return []
        ...
        for email_id in email_ids:
            try:
                ...
                try:
                    ...
                    st.warning(f"Failed to parse date: {str(e)}")
                ...
                for part in msg.walk():
                    ...
                    st.success(f"Found attachment: {filename}")
...
def process_email_bills(username, email_account, password):
    ...
    try:
        ...
        last_checked = datetime.now() - timedelta(days=30)  # Default: check last 30 days
